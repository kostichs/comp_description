...
# Base directory - исправлено для Windows
...
# Input files - обновлено для новой структуры
INPUT_PATH = os.path.join(BASE_DIR, "input", "results_250.csv")
...
# Processing limits
COMPANIES_LIMIT = 10  # Ограничение для тестирования новых функций
...
def validate_config():
    ...
    if not os.path.exists(CRITERIA_DIR):
        missing.append(f"CRITERIA_DIR: {CRITERIA_DIR}")
    ...
    if PROCESSING_CONFIG['enable_hubspot_integration'] and not os.getenv("HUBSPOT_API_KEY"):
        print("⚠️  Warning: HubSpot integration enabled but no HUBSPOT_API_KEY found")
    
    print(f"✅ Configuration validated for criteria type: {CRITERIA_TYPE}")
    print(f"   Criteria directory: {CRITERIA_DIR}")
    print(f"   Companies file: {INPUT_PATH}")
